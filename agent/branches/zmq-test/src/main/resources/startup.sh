...
AGENT_NAME=com.hhi.vaas.platform.agent.AgentMain

PID=`ps -ef | grep java | grep "$AGENT_NAME" | awk '{print $2}'`

if [ e$PID != "e" ] ; then
    echo "VDIP Agent is already RUNNING..."
    exit;
fi
...
nohup $VDIP_JAVA -cp ./conf:lib/* -server -Xms256m -Xmx256m $AGENT_NAME > /dev/null 2>&1 &
