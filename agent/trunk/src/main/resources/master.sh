...
AGENT_NAME=com.hhi.vaas.platform.agent.master.MasterMain

PID=`ps -ef | grep java | grep "$AGENT_NAME" | awk '{print $2}'`

if [ e$PID != "e" ] ; then
    echo "VDIP Agent Master is already RUNNING..."
    exit;
fi
...
nohup $VDIP_JAVA -cp ./conf:vdip-agent-master-1.0.0-jar-with-dependencies.jar $AGENT_NAME > /dev/null 2>&1 &
